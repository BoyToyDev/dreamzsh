...
установить его через поддерживаемый пакетный менеджер Linux или macOS и сделать
оболочкой входа. После установки:
...
Перед изменением локальных данных DreamZSH сверяет полный список файлов и
контрольные суммы SHA-256. Если установка прервётся, импорт целиком откатывается.
Профили содержат исполняемый код тем и плагинов, поэтому импортируйте только
наборы от людей, которым доверяете.

## 💾 Резервные копии

В резервную копию входят конфигурация и пользовательские ресурсы из `custom/`.
Файлы самого DreamZSH не перезаписываются при восстановлении.

```zsh
dreamzsh backup create --all
dreamzsh backup list
dreamzsh backup restore <архив>
```
...
| Сохранить резервную копию | `dreamzsh backup create --all` |
